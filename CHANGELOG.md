...
### v1.2.0 (2024-12-20)
**文件导入数据样式一致性和配置保持重大修复版本**

### 🔧 重大功能修复

#### 文件导入数据样式不一致问题修复
- **问题修复**: 修复了从CSV/Excel文件导入数据后前端样式与模拟数据不一致的问题
- **根本原因**: 文件导入使用了固定样式的HTML生成方法，而模拟数据使用了配置化的HTML生成方法
- **技术方案**: 统一使用`_create_local_heatmap_html_with_config`方法以确保样式一致性

#### 配置变更自动切回模拟数据问题修复
- **问题修复**: 修复了点击配置项会自动切回模拟数据显示的问题
- **根本原因**: 文件导入时没有保存当前图表状态（`current_chart_data`等属性）
- **技术方案**: 在`render_file_heatmap`方法中添加状态保存逻辑

#### 修复详情
- 🎯 **状态保存**: 文件导入时正确保存`current_chart_data`、`current_chart_type`、`current_chart_name`
- 🎯 **样式统一**: 所有数据源（模拟数据、CSV、Excel）现在都使用相同的样式生成逻辑
- 🎯 **配置保持**: 配置变更时能够保持导入的数据显示，不再切回模拟数据
- 🎯 **颜色同步**: 导入数据的颜色方案与UI配置面板完全同步
- 🎯 **兼容性增强**: 改进了不同数据类型的处理逻辑

### 🎨 用户体验改进
- 导入数据后的视觉效果与模拟数据完全一致
- 配置面板的所有设置都能正确应用到导入的数据
- 用户可以在导入数据后自由调整样式配置而不丢失数据
- 提供了更加统一和直观的用户交互体验
...
#### 视觉映射颜色条显示失效问题修复
- **问题修复**: 修复了视觉映射配置中"显示颜色条"复选框无效的问题
- **根本原因**: ECharts配置中的`visualMap`缺少`show`属性，导致UI控制失效
- **技术方案**: 在`_build_echarts_config_from_ui`方法中添加`show`属性配置

#### 修复详情
- 🎯 修复了`_build_echarts_config_from_ui`方法中`visualMap`配置缺少`show`属性的问题
- 🎯 确保UI中的"显示颜色条"复选框能正确控制颜色条的显示/隐藏
- 🎯 视觉映射配置现在能响应用户的UI操作
- 🎯 颜色条显示状态与配置面板同步
